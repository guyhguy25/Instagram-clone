...
function Post({ postId, user, username, caption, imageUrl, likes, userslike}) {
    ...
    return (
        <div className="post">
            <div className="post__header">
                <a className="usernamea" href={`/${username}`}>
                    <Avatar className="post__avatar" alt={profileData?.userpic} src={profileData?.userpic}/>
                </a>
                <h3><a className="usernamea" href={`/${username}`} style={{fontSize: "16px"}}><strong>{username}</strong></a></h3>
            ...
            {likes < 1 ? (<span className="post__text">be the first like</span>) : 
            (<button className="likes__button" onClick={handleOpen}>
                <span className="post__text"><strong>{likes} likes</strong></span></button>)}
            ...
            <div className="post__comments">
                <span className="caption_title">
                    <strong><a title={username} href={`/${username}`}>{username}</a></strong> {caption}
                </span>
            ...
            {user && (
                <form className="post__commentBox">
                    <input id={postId} className="post__input" type="text" placeholder="Add a comment..." value={comment} onChange={(e) => setComment(e.target.value)}/>
